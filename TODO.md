...
- [x] Реализовать систему меню согласно analyze-prj.md
  - [x] Этап 1: Базовая структура системы меню
  - [x] Этап 2: Обработка кнопок и роутинг
  - [x] Этап 3: Интеграция с командой /start
  - [x] Этап 4: Реализация управления меню
  - [x] Этап 5: Интеграция в плагины

## Выполненные задачи (2025-09-27)

### Система меню (2025-09-27):
- [x] **Этап 1: Базовая структура**
  - [x] Создан файл `menu_system.py` с базовыми компонентами
  - [x] Реализован класс `MenuEntry` для хранения информации о пунктах меню

### Добавление глобальной кнопки меню (2025-09-27):
- [x] [feat] Создан плагин `menu_keyboard` для добавления постоянной кнопки "Меню" под строкой ввода в Телеграме
- [x] Добавлена поддержка ReplyKeyboardMarkup для создания клавиатуры с кнопкой
- [x] Добавлены локализации кнопки на русском, английском, татарском и башкирском языках
- [x] Реализован автоматический показ клавиатуры при команде `/start`
- [x] Создана команда `/keyboard` для ручного добавления клавиатуры
- [x] Создан обработчик нажатия на кнопку "Меню" для вызова главного меню бота
- [x] Добавлена регистрация плагина в системе меню
  - [x] Добавлены глобальные структуры `MENU_REGISTRY` и `MENU_CACHE`
  - [x] Реализована функция `register_menu()` для регистрации пунктов меню
  - [x] Реализована функция `build_menu()` для построения кнопок

- [x] **Этап 2: Обработка кнопок и роутинг**
  - [x] Улучшена функция `dispatch_command` с проверкой прав доступа и обработкой ошибок
  - [x] Расширена функция `init_menu_system` для обработки callback-кнопок с параметрами
  - [x] Добавлен обработчик текстовых сообщений, соответствующих пунктам меню

- [x] **Этап 3: Интеграция с командой /start**
  - [x] Модифицирован плагин `start_cmd.py` для использования системы меню
  - [x] Добавлены ключи локализации для меню в `ru.json`
  - [x] Создан тестовый плагин `menu_test` для демонстрации работы системы

- [x] **Этап 4: Реализация управления меню**
  - [x] Расширен класс `MenuEntry` с поддержкой флага `enabled` и произвольных данных
  - [x] Доработана функция `invalidate_menu` с гибкими опциями фильтрации
  - [x] Реализована функция `refresh_menu` для обновления меню без перезапуска бота
  - [x] Добавлена функция `toggle_menu_item` для управления видимостью пунктов
  - [x] Создан плагин `menu_admin` для административного управления меню

- [x] **Этап 5: Интеграция в плагины**
  - [x] Добавлена интеграция с плагином `privet`
  - [x] Добавлена интеграция с плагином `setlang`
  - [x] Добавлена интеграция с плагином `inline_button`
  - [x] Обновлены ключи локализации для всех интегрированных плагинов

### Локализация башкирский (2025-09-27):
- [x] [i18n] Переведены базовые и админские сообщения в `ba.json` (были на русском)

### Фиксы локализации меню (2025-09-27):
- [x] [fix] После смены языка через /setlang теперь пересоздаётся reply-клавиатура с обновлённой кнопкой "Меню" без необходимости повторного /start
 - [x] [fix] Защита вызовов event.answer в menu_system (AttributeError при меню 'keyboard' через текстовое сообщение) — добавлен _safe_notify()

### Улучшения UX меню (2025-09-27):
- [x] [feat] Авто-скрытие inline-меню после нажатия любой кнопки (кроме back) — удаление кнопок через edit(buttons=None)
